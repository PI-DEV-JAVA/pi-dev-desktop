...
public class ProfileViewController {
    ...
    public void initialize() {
        ...
        // Build completion bar with action buttons
    ...
    // ===== COMPLETION BAR WITH ACTION BUTTONS =====
    ...
    private void buildCompletionBar(Profile profile, User user) {
        ...
        // Milestone steps — each is a row with icon, label, status, and action button
        ...
        steps.getChildren().add(createMilestoneRow("📸", "Upload Profile Picture", "+10%", hasPicture,
                hasPicture ? null : createActionBtn("Upload Photo", "#8b5cf6", e -> handleUploadPhoto(profile, user))));
        ...
        steps.getChildren().add(createMilestoneRow("📄", "Upload CV", "+10%", hasCv,
                hasCv ? null : createActionBtn("Upload CV", "#22c55e", e -> handleUploadCV(profile, user))));
    ...
    private HBox createMilestoneRow(String icon, String label, String pctText, boolean completed, Button actionBtn) {
        ...
        row.setStyle("-fx-background-color: " + (completed ? "#f0fdf4" : "#fafbfc") +
    ...
    private void handleUploadPhoto(Profile profile, User user) {
        ...
        if (file != null) {
            ...
            profile.setProfilePicturePath(file.getAbsolutePath());
    ...
    private void handleUploadCV(Profile profile, User user) {
        FileChooser chooser = new FileChooser();
    ...
    private void handleVerifyEmail(User user) {
        ...
        sending.setTitle("Sending...");
        sending.setHeaderText(null);
        sending.setContentText("Sending verification code to " + user.getEmail() + "...");
        ...
        if (result.isPresent()) {
            ...
            if (entered.equals(code)) {
                ...
                showAlert("❌ Invalid Code", "The code you entered doesn't match. Please try again.",
    ...
    private void handleDelete() {
        ...
        alert.setContentText("This will permanently remove your profile details.");
