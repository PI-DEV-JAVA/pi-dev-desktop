...
public class ApplicationsCardController implements Initializable {
    ...
    private void filterApplications() {
        ...
        for (Application app : applicationsList) {
            ...
            // Filtre par mot-clé
            if (!keyword.isEmpty()) {
                String displayName = getCandidateDisplayName(profileDao, app.getUserId());
                matches = displayName.toLowerCase().contains(keyword) ||
                        String.valueOf(app.getUserId()).contains(keyword);
            }
    ...
    private VBox createApplicationCard(Application app) {
        ...
        ProfileDao profileDao = new ProfileDao();
        String candidateName = getCandidateDisplayName(profileDao, app.getUserId());
        ...
        Label emailLabel = new Label("👤 User #" + app.getUserId());
        ...
        Profile candidateProfile = profileDao.findByUserId(app.getUserId());
        if (candidateProfile != null && candidateProfile.getPhoneNumber() != null
                && !candidateProfile.getPhoneNumber().isEmpty()) {
            Label phoneLabel = new Label("📞 " + candidateProfile.getPhoneNumber());
    ...
    private void showApplicationDetails(Application app, Offer offer) {
        ...
        alert.setHeaderText(candidateName);
    ...
    private void showEvaluationDialog(Application app) {
        ...
        // Créer le dialogue
        ProfileDao evalProfileDao = new ProfileDao();
        String evalCandidateName = getCandidateDisplayName(evalProfileDao, app.getUserId());
        Profile evalProfile = evalProfileDao.findByUserId(app.getUserId());

        Dialog<Application> dialog = new Dialog<>();
        dialog.setTitle("Évaluation de candidature");
        dialog.setHeaderText("Évaluation de " + evalCandidateName);
        ...
        String initials = getInitials(evalCandidateName);
        Label initialsLabel = new Label(initials);
        ...
        Label nameLabel = new Label(evalCandidateName);
        nameLabel.setStyle("-fx-font-size: 20px; -fx-font-weight: bold; -fx-text-fill: #F1F5F9;");

        String evalPhone = (evalProfile != null && evalProfile.getPhoneNumber() != null) ? evalProfile.getPhoneNumber()
                : null;
        if (evalPhone != null && !evalPhone.isEmpty()) {
            Label phoneLabel = new Label("📞 " + evalPhone);
            phoneLabel.setStyle("-fx-font-size: 13px; -fx-text-fill: #94A3B8;");
            candidateInfo.getChildren().addAll(nameLabel, phoneLabel);
        } else {
            candidateInfo.getChildren().addAll(nameLabel);
        }
        ...
        scoreSlider.valueProperty()
                .addListener((obs, old, newVal) -> scoreValue.setText(String.format("%.0f", newVal)));
    ...
    private void deleteApplication(Application app) {
        ProfileDao delProfileDao = new ProfileDao();
        String delName = getCandidateDisplayName(delProfileDao, app.getUserId());
        Alert confirm = new Alert(Alert.AlertType.CONFIRMATION);
        confirm.setTitle("Confirmation");
        confirm.setHeaderText("Supprimer la candidature");
        confirm.setContentText("Voulez-vous vraiment supprimer la candidature de " + delName + " ?");
    ...
    private String getCandidateDisplayName(ProfileDao profileDao, int userId) {
