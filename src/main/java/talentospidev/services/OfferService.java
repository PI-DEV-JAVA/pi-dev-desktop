...
import talentospidev.dao.OfferDAO;
...
public class OfferService {
    ...
    public List<Offer> getOpenOffers() {
        try {
            return offerDAO.getOpenOffers();
        } catch (SQLException e) {
            System.err.println("Erreur lors de la récupération des offres ouvertes: " + e.getMessage());
            return List.of();
        }
    }

    public List<Offer> getOffersByRecruiterId(int recruiterId) {
        try {
            return offerDAO.getOffersByRecruiterId(recruiterId);
        } catch (SQLException e) {
            System.err.println("Erreur lors de la récupération des offres du recruteur: " + e.getMessage());
            return List.of();
        }
    }
    ...
    public int getTotalOffersByRecruiter(int recruiterId) {
        try {
            return offerDAO.getTotalOffersByRecruiter(recruiterId);
        } catch (SQLException e) {
            return 0;
        }
    }

    public int getTotalApplicationsByRecruiter(int recruiterId) {
        try {
            return offerDAO.getTotalApplicationsByRecruiter(recruiterId);
        } catch (SQLException e) {
            return 0;
        }
    }
